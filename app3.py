...
# ─────────────────────────────────────────────────────────────
# SIDEBAR REAJUSTADA PARA ELIMINAR EL "BLOQUEO DE ESTADO"
# ─────────────────────────────────────────────────────────────
with st.sidebar:
    ...
    for i, c in enumerate(cfgs):
        with st.expander(f"📄 Sección {i+1}", expanded=(n <= 5)):
            ...
            a, b = st.columns(2)
            c["p0"] = a.number_input("Pág. Inicio", min_value=1, max_value=tp, key=k_p0)
            c["p1"] = b.number_input("Pág. Fin", min_value=c["p0"], max_value=tp, key=k_p1)
            
            c["det_iva"] = st.checkbox("Detectar IVA", key=k_iv)
            c["calc_sub"] = st.checkbox("Calcular subtotal si falta", key=k_cs)
