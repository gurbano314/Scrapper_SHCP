import hashlib, io, re, os, yaml, pathlib, tempfile, datetime
...
# Filas de cotización con espacio/tab: "1  20  pza  Playeras Polo  300.00  6,000.00"
_ITEM_RE = re.compile(
...
# ── OCR con rutas absolutas (fix Cloud) ───────────────────────
# ROOT-CAUSE: rapidocr config.yaml usa rutas RELATIVAS a los modelos.
# En Streamlit Cloud el CWD es /mount/src/<repo>, no el directorio del paquete.
# → Se genera un config temporal con rutas absolutas antes de instanciar RapidOCR.
@st.cache_resource(show_spinner=False)
def _ocr_engine():
    try:
        import rapidocr_onnxruntime as _r
        pkg     = pathlib.Path(_r.__file__).parent
        cfg_src = pkg / "config.yaml"
        cfg     = yaml.safe_load(cfg_src.read_text())

        # Parchear rutas relativas → absolutas
        for sec in ("Det", "Rec", "Cls"):
            rel = cfg[sec]["model_path"]
            abs_p = str(pkg / rel)
            if not pathlib.Path(abs_p).exists():
                return None          # modelos no disponibles
            cfg[sec]["model_path"] = abs_p

        # Escribir config temporal con rutas absolutas
        ...
        return RapidOCR(config_path=tmp.name)
    except Exception:
        return None
...
def _ocr_page(pdf_bytes: bytes, idx: int) -> str:
    """OCR con config de rutas absolutas; nunca crashea."""
    ocr = _ocr_engine()
    if ocr is None:
        return ""
    try:
...
# ── Parser de tablas alineadas con espacios/tabs ──────────────
def _parse_space_table(text: str) -> list[dict]:
    """
    Detecta filas de cotización sin bordes usando regex posicional.
    Formato: [part#]  [qty]  [unidad]  [descripción]  [P.U.]  [total]
    Ejemplo: '1  20  pza  Playeras Polo  300.00  6,000.00'
    """
    items = []
...
def extract(
    ...
    Nivel 3 – OCR (páginas escaneadas)
    """
    text, table_rows = "", []
    ...
    with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf:
        ...
        for i in pr:
            ...
            for tbl in pg.extract_tables():
                if tbl:
                    table_rows.extend(
                        [str(c or "").strip() for c in r] for r in tbl if r
                    )

            # Tablas sin bordes alineadas con espacios/tabs (ej. Desportik)
            for item in _parse_space_table(txt):
                # Convertir cada ítem a una fila compatible con la lógica existente
                table_rows.append([
                    str(item["qty"]), item["desc"],
                    f"${item['pu']:,.2f}", f"${item['total']:,.2f}"
                ])

    # Nivel 3: OCR si el texto es insuficiente
    ...
    # (ej. Desportik: 6,000 + 9,000 = 15,000 pero no hay fila total explícita)
    if tot is None and table_rows:
        ...
        if item_totals:
            candidate = sum(item_totals)
            # Solo tomar si hay 2+ ítems (evitar falsos positivos)
            if len(item_totals) >= 2:
                tot = round(candidate, 2)

    # ── Fallback texto libre ───────────────────────────────────
    if tot is None:
        for ln in text.splitlines():
            if re.search(r"\btotal\b", ln, re.I) and \
               not re.search(r"sub|parcial", ln, re.I):
                v = _money(ln)
                if v and (tot is None or v > tot): tot = v
    ...
    if tot and not sub and not iva and iva_f == "Sí":
        sub = round(tot / 1.16, 2); iva = round(tot - sub, 2)
    ...
    return {
        "Fecha":                  (fecha.isoformat() if fecha
...
with st.sidebar:
    ...
    for i, c in enumerate(cfgs):
        with st.expander(f"📄 Sección {i+1}", expanded=(n <= 5)):
            ...
            c["p1"]       = b.number_input("Pág. Fin", c["p0"], tp,
                                           max(min(c["p1"], tp), c["p0"]), key=f"p1{i}")
            c["det_iva"]  = st.checkbox("Detectar IVA", value=c["det_iva"],  key=f"iv{i}")
            c["calc_sub"] = st.checkbox("Calcular subtotal si falta",
                                        value=c["calc_sub"], key=f"cs{i}")
...
if run and st.session_state.pdf_bytes:
    ...
    for i, c in enumerate(st.session_state.sec_cfg):
        ...
        try:
            rows.append(extract(
                st.session_state.pdf_bytes, c["label"],
    ...
    df_new["Diferencia final"] = (
        df_new["Total con IVA"].fillna(0)
        - df_new["Monto en Anexo Escrito"].fillna(0)
    ).where(
        df_new["Total con IVA"].notna() | df_new["Monto en Anexo Escrito"].notna()
    )
...
with L:
    ...
    cp = int(c2.number_input("", 1, tp, cp + 1,
                             label_visibility="collapsed", key="pg")) - 1
...
with R:
    ...
    if not st.session_state.extracted or st.session_state.df is None:
        ...
        for col, val, lbl in [
            ...
            clr = "#1a2744" if lbl != "Diferencia" \
                  else ("#c0392b" if abs(dif) > .01 else "#28a745")
            col.markdown(
                f'<div class="kpi"><div class="v" style="color:{clr}">'
        ...
        ed = st.data_editor(
            ...
            column_config={
                ...
                "Total con IVA":          st.column_config.NumberColumn(
                                              "Total", format="$%.2f", width="medium"),
                "Diferencia final":       st.column_config.NumberColumn(
                                              "Diferencia", format="$%.2f", width="medium"),
                "Monto en Anexo Escrito": st.column_config.NumberColumn(
                                              "Ref. Escrito", format="$%.2f", width="medium"),
