# ============================================================
# EXTRACTOR DE COTIZACIONES PDF | app.py  v2.1
# ============================================================
...
#   Grupo 2 (sin $): exige separador de miles → 1,234.56 ✓
#   Grupo 3 (sin $): números ≥4 dígitos sin comas → 12345.67 ✓ (CORRECCIÓN)
_MONEY_RE = re.compile(
    r"\$\s*([\d,]+(?:\.\d{1,2})?)"                     # con $
    r"|(?<!\d)([\d]{1,3}(?:,\d{3})+(?:\.\d{1,2})?)"   # con comas
    r"|(?<!\d)(\d{4,}(?:\.\d{1,2})?)(?!\d)"            # sin comas, ≥4 dígitos
)
...
def _money(txt: str) -> Optional[float]:
    ...
    for m in _MONEY_RE.finditer(str(txt)):
        raw = m.group(1) or m.group(2) or m.group(3)   # ahora incluye el grupo 3
        if raw:
            try:
    ...
    if tot is None:
        ...
        for ln in text.splitlines():
            for m in _MONEY_RE.finditer(ln):
                raw = m.group(1) or m.group(2) or m.group(3)   # CORREGIDO
                if raw:
                    try:
                        v = float(raw.replace(",", ""))
                        if v >= 10.0:
...
with st.sidebar:
    ...
    # ── Manejo seguro de sec_cfg (sin mutación in‑place) ──
    cfgs = list(st.session_state.sec_cfg)          # copia defensiva
    ...
    # Asignar la nueva lista → Streamlit detecta el cambio
