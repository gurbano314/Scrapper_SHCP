# ============================================================
# EXTRACTOR DE COTIZACIONES PDF | app.py  v2.1 (refactor)
# ============================================================
...
def _clamp_page_idx(pdf_bytes: bytes, idx: int) -> int:
    try:
...
def _ocr_page(pdf_bytes: bytes, idx: int) -> str:
    """Extrae texto de una página vía OCR (fallback cuando el texto nativo es escaso)."""
    ocr = _get_ocr_engine()
    if ocr is None:
        return ""

    idx = _clamp_page_idx(pdf_bytes, idx)
...
# ─────────────────────────────────────────────────────────────
# RE-CÁLCULO DE CAMPOS DERIVADOS
# ─────────────────────────────────────────────────────────────
def recalc_derived(df: pd.DataFrame) -> pd.DataFrame:
    """
    Recalcula 'Diferencia final' = Total con IVA − Monto en Anexo Escrito.
    Usa aritmética decimal y conserva NaN cuando ambos campos faltan.
    """
    df = df.copy()

    def _diff(row) -> Optional[float]:
        tot = _decimal(row.get("Total con IVA"))
        anex = _decimal(row.get("Monto en Anexo Escrito"))
        if tot is None and anex is None:
            return None
        tot = tot or Decimal("0")
        anex = anex or Decimal("0")
        return float((tot - anex).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP))

    if "Diferencia final" not in df.columns:
        df["Diferencia final"] = None
    df["Diferencia final"] = df.apply(_diff, axis=1)
    return df
...
def extract(
    ...
    """
    Extrae datos financieros de p0..p1 usando:
      Nivel 1: texto nativo (pdfplumber)
      Nivel 2: tablas estructuradas
      Nivel 3: OCR condicional si el texto nativo es insuficiente
    """
    native_text = ""
    ...
    for row in table_rows:
        ...
        if "total" in j and not re.search(r"sub|parcial|acum", j):
            v = _money(s)
            if v is not None and (tot is None or v > tot):
                tot = v
        if re.search(r"\biva\b|16%|vat", j):
            v = _money(s)
            if v is not None and (iva is None or v > iva):
                iva = v
        if re.search(r"subtotal|sin\s*iva|importe\s*neto", j):
            v = _money(s)
            if v is not None and (sub is None or v > sub):
                sub = v
    ...
    if tot is not None and sub is None and iva is None and iva_f == "Sí":
        ...
        tot = round(sub + iva, 2)
    elif calc_sub and sub is not None and iva is None and tot is None and iva_f == "Sí":
...
def to_excel(df: pd.DataFrame) -> bytes:
    ...
    for i, (_, row) in enumerate(df.iterrows()):
        ...
        if isinstance(fecha_val, (datetime.date, datetime.datetime)):
            dt = (
                datetime.datetime.combine(fecha_val, datetime.time())
                if isinstance(fecha_val, datetime.date) and not isinstance(fecha_val, datetime.datetime)
                else fecha_val
            )
...
def _parse_json_loose(text: str) -> dict:
    """
    Intenta recuperar un JSON aunque venga encapsulado en markdown o con ruido.
    """
...
with st.sidebar:
    ...
    # Copia defensiva para evitar mutación accidental por referencia
    cfgs = [dict(c) for c in st.session_state.sec_cfg]
...
with col_L:
    ...
    if new_page - 1 != cp:
        st.session_state.current_page = int(new_page) - 1
        cp = st.session_state.current_page
    ...
    with st.spinner("Cargando…"):
        try:
            st.image(
                _render(st.session_state.pdf_hash or "", cp),
                use_container_width=True,
            )
        except Exception as exc:
            st.error(f"No se pudo renderizar la página: {exc}")
...
with col_R:
    ...
    if not st.session_state.extracted or st.session_state.df is None:
        ...
        if edited_df is not None:
            new_df = recalc_derived(edited_df)
            # Evita asignaciones redundantes que disparan reruns innecesarios
        ...
        if st.button("🤖 Auditar montos con IA", type="secondary", use_container_width=True):
            if genai is None:
                st.error("No se pudo importar google.generativeai en este entorno.")
                    ...
                    try:
                        ...
                        for m_name in _GEMINI_PREFERRED:
                            try:
                                ...
                                # Validación ligera de disponibilidad
...
{{
  ...
  "discrepancias": [
    {{"rubro": "...", "campo": "...", "valor_extraido": 0.0, "valor_correcto": 0.0}}
                            ...
                            result = _parse_json_loose(gemini_resp_text)
